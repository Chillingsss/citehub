...
import { checkPasswordReset, verifyPasswordResetOTP } from "../utils/security";
...
export default function PasswordResetModal({
	...
	const [step, setStep] = useState(isRequired ? 2 : 1); // Start at step 2 (password reset) if password reset is required
	...
	const handleCheckReset = async () => {
		...
		try {
			...
			if (data.success && data.needsReset) {
				setUserEmail(data.user_email);
				setStep(2); // Go directly to password reset
				toast.success("Password reset required. Please create a new password.");
	...
	const handleResetPassword = async () => {
		...
		try {
			const result = await verifyPasswordResetOTP(username, "", newPassword);
			const data = typeof result === "string" ? JSON.parse(result) : result;
